---
source: https://platform.claude.com/docs/en/agent-sdk/
title: Anthropic Agent SDK Documentation Index
last_fetched: 2026-02-12T10:05:14.701275+00:00
---

# Anthropic Agent SDK Documentation

This is a local mirror of the [Anthropic Agent SDK documentation](https://platform.claude.com/docs/en/agent-sdk/).

## Available Pages

- [Agent SDK overview](./overview.md)
- [Agent SDK reference - Python](./python.md)
- [Agent SDK reference - TypeScript](./typescript.md)
- [Agent Skills in the SDK](./skills.md)
- [Configure permissions](./permissions.md)
- [Connect to external tools with MCP](./mcp.md)
- [Custom Tools](./custom-tools.md)
- [Get structured output from agents](./structured-outputs.md)
- [Handle approvals and user input](./user-input.md)
- [Hosting the Agent SDK](./hosting.md)
- [Intercept and control agent behavior with hooks](./hooks.md)
- [Migrate to Claude Agent SDK](./migration-guide.md)
- [Modifying system prompts](./modifying-system-prompts.md)
- [Plugins in the SDK](./plugins.md)
- [Quickstart](./quickstart.md)
- [Rewind file changes with checkpointing](./file-checkpointing.md)
- [Securely deploying AI agents](./secure-deployment.md)
- [Session Management](./sessions.md)
- [Slash Commands in the SDK](./slash-commands.md)
- [Subagents in the SDK](./subagents.md)
- [Tracking Costs and Usage](./cost-tracking.md)
- [TypeScript SDK V2 interface (preview)](./typescript-v2-preview.md)

## About This Mirror

This documentation mirror is automatically synced from the official Anthropic documentation.
Claude Code agents can reference these files directly for up-to-date SDK information.

**Last sync:** 2026-02-12T10:05:14.701275+00:00

To manually sync, run:
```bash
python scripts/sync_anthropic_docs.py
```
