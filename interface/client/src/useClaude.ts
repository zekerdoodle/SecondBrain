...
import type { ChatMessage, ChatImageRef, MessageStatus, ChatDisplayPayload, ChatHelperSettings } from './types';
...
export const normalizeHelperSettings = (value: unknown): ChatHelperSettings => {
  ...
  const titler = raw.titler && typeof raw.titler === 'object' ? raw.titler as Record<string, any> : {};
  const memoryRaw = raw.contextual_memory ?? raw.contextualMemory;
  const memory = memoryRaw && typeof memoryRaw === 'object' ? memoryRaw as Record<string, any> : {};
  const mode = memory.mode === 'off' || memory.mode === 'manual' ? memory.mode : 'auto';
  const manualQuery = typeof memory.manual_query === 'string'
    ? memory.manual_query
...
export interface ClaudeHook {
  ...
  sendMessage: (text: string, images?: ChatImageRef[], displayPayload?: ChatDisplayPayload) => boolean;
  ...
  helperSettings: ChatHelperSettings;
  updateHelperSettings: (settings: ChatHelperSettings) => Promise<void>;
}
...
export const useClaude = (options: ClaudeOptions = {}): ClaudeHook => {
  ...
  const [helperSettings, setHelperSettings] = useState<ChatHelperSettings>(() => normalizeHelperSettings(null));
  const helperSettingsRef = useRef<ChatHelperSettings>(helperSettings);
  ...
  useEffect(() => { helperSettingsRef.current = helperSettings; }, [helperSettings]);
          ...
          fetch(`${API_URL}/chat/history/${sessionId}`)
            ...
            .then(data => {
              ...
              if (awaitingStateResponse.current && msgs && Array.isArray(msgs)) {
                ...
                setMessages(msgs);
                setHelperSettings(normalizeHelperSettings(data.helper_settings));
              }
              isUserInitiatedLoad.current = false;
            })
            .catch(err => {
  ...
  // Tool-use stale state safety net
  // Thinking/processing completion is owned by authoritative server events
  // (done/interrupted/error/server idle/etc.), not a blind client timeout.
  // Keep the old long timeout only for tool_use so abandoned tool UI can still
  // clean itself up without hiding legitimate long model calls.
  const processingTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const lastActivityTime = useRef<number>(Date.now());

  // Track activity for stream stall detection and tool-use timeout rescheduling.
  useEffect(() => {
    lastActivityTime.current = Date.now();
  }, [status, statusText, toolName]);

  useEffect(() => {
    // Clear any existing timeout whenever status changes.
    ...
    const timeoutMs = 300000; // 5 minutes for stale tool-use UI only
    ...
    const scheduleTimeout = () => {
      processingTimeoutRef.current = setTimeout(() => {
        ...
        if (timeSinceActivity < timeoutMs - 1000) {
          ...
          console.log(`Tool-use activity detected ${timeSinceActivity}ms ago, rescheduling timeout in ${remaining}ms`);
          processingTimeoutRef.current = setTimeout(() => {
            const recheck = Date.now() - lastActivityTime.current;
            if (recheck < timeoutMs - 1000) {
              console.log(`Tool-use still active after reschedule (${recheck}ms ago), deferring`);
              return;
            }
            console.log(`Tool-use timeout after reschedule - resetting to idle.`);
            setStatus('idle');
            setStatusText('');
            setActiveTools(new Map());
        ...
        console.log(`Tool-use timeout after ${timeoutMs/1000}s - resetting to idle (likely stale state).`);
        setStatus('idle');
        setStatusText('');
        setActiveTools(new Map());
  ...
  }, [status]); // Re-run when status changes - tool timeout resets on each status change
  ...
  const sendMessageInternal = useCallback((text: string, images?: ChatImageRef[], agent?: string, displayPayload?: ChatDisplayPayload): boolean => {
    if (!ws.current || ws.current.readyState !== WebSocket.OPEN) {
    ...
    setMessages(prev => [...prev, {
      id: msgId,
      role: 'user',
      content: displayPayload?.displayContent ?? text,
      displayContent: displayPayload?.displayContent,
      displaySegments: displayPayload?.displaySegments,
      replyReferences: displayPayload?.replyReferences,
      agentContent: displayPayload ? text : undefined,
      status: 'pending',
      timestamp: Date.now(),
      images: images,
    }]);
    ...
    const wsPayload: Record<string, any> = {
      ...
      sessionId,
      message: text,
      displayMessage: displayPayload?.displayContent,
      displaySegments: displayPayload?.displaySegments,
      replyReferences: displayPayload?.replyReferences,
      msgId,
      helperSettings: helperSettingsRef.current,
  ...
  const injectMessage = useCallback((text: string, displayPayload?: ChatDisplayPayload): boolean => {
    if (!ws.current || ws.current.readyState !== WebSocket.OPEN) {
  ...
  const sendMessageWithAgent = useCallback((text: string, agent?: string, images?: ChatImageRef[], displayPayload?: ChatDisplayPayload): boolean => {
    if (status !== 'idle') {
      return injectMessage(text, displayPayload);
    }
    return sendMessageInternal(text, images, agent, displayPayload);
  ...
  const sendMessage = useCallback((text: string, images?: ChatImageRef[], displayPayload?: ChatDisplayPayload): boolean => {
    ...
    return sendMessageInternal(text, images, undefined, displayPayload);
  }, [status, sendMessageInternal, injectMessage]);
  ...
  const startNewChat = useCallback(() => {
    ...
    setHelperSettings(normalizeHelperSettings(null));
    pendingDeltas.current.clear();
  ...
  const updateHelperSettings = useCallback(async (settings: ChatHelperSettings): Promise<void> => {
    const normalized = normalizeHelperSettings(settings);
    setHelperSettings(normalized);
    helperSettingsRef.current = normalized;
    ...
    try {
      const res = await fetch(`${API_URL}/chat/history/${sessionId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ helper_settings: normalized }),
      });
      if (!res.ok) {
        console.warn('Failed to persist helper settings:', await res.text());
      }
    } catch (e) {
      console.warn('Failed to persist helper settings:', e);
    }
  }, [sessionId]);
